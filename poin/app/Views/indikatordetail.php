          ...
          <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            <div class="card">
              <form action="<?php echo base_url('indikator/ubah') ?>" method="post">
                <input type="hidden" name="id" value="<?php echo $data['idindikator'] ?>"> 
                <div class="card-body">
                  <div class="form-group">
                    <label for="inputSmall" class="col-form-label">Indikator</label>
                    <input type="text" name="indikator" class="form-control form-control-sm" placeholder="Nama Indikator" required value="<?php echo $data['indikator'] ?>">
                  </div>
                  <div class="form-group">
                    ...
                    <select class="form-control form-control-sm" name="kriteria" required>
                      <?php foreach ($kriteria as $k) {?>
                        <option value="<?php echo $k['idkriteria'] ?>" <?php if($data['idkriteria'] == $k['idkriteria']){echo "selected";} ?>><?php echo $k['kriteria']." (".$k['kategori'].")" ?></option>
                      <?php } ?>
                    </select>
                  </div>
                  <div class="form-group">
                    <label for="inputSmall" class="col-form-label">Nilai</label>
                    <input type="number" name="nilai" class="form-control form-control-sm" min="1" max="100" value="<?php echo $data['nilai'] ?>" placeholder="Nilai Indikator" required>
                  </div>
                </div>
